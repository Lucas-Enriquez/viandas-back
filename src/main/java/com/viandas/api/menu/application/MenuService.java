...
import com.viandas.api.menu.persistence.*;
...
public class MenuService {
	...
	public List<MenuResponse> list(CurrentUser currentUser, Long companyId, LocalDate date) {
		...
		if (companyId != null && date != null) {
			menus = menuRepository.findByCompanyCookIdAndCompanyIdAndMenuDateOrderByIdDesc(currentUser.userId(), companyId, date);
		} else if (companyId != null) {
			menus = menuRepository.findByCompanyCookIdAndCompanyIdOrderByMenuDateDescIdDesc(currentUser.userId(), companyId);
		} else if (date != null) {
			menus = menuRepository.findByCompanyCookIdAndMenuDateOrderByIdDesc(currentUser.userId(), date);
		} else {
			menus = menuRepository.findByCompanyCookIdOrderByMenuDateDescIdDesc(currentUser.userId());
		}
